...
class BTCTurkAPI(BaseAPI):
    ...
    async def get_btc_try_price(cls) -> Optional[float]:
        ...
        try:
            ...
            pairs_found = []
            for pair_data in response_data['data'][:5]:
    ...
    async def get_usdt_try_rate(cls) -> Optional[float]:
        ...
        try:
            ...
            # Debug: Log the first few pairs in the response
            pairs_found = []
            for pair_data in response_data['data'][:5]:
                if isinstance(pair_data, dict):
                    pair = pair_data.get('pair', 'unknown')
            ...
            all_pairs = [p.get('pair') for p in response_data.get('data', []) if isinstance(p, dict)]
            logger.info(f"All pairs found: {all_pairs}")
            return None
